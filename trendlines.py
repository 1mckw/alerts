...
wick reaches or nears the line (0% near tolerance for alerts). Sharp up/down bars that break the line are
...
def count_line_touch_points(
    ...
    """Touch points: strict pivots on line + local extrema with wick on/near line (0% near)."""
    if start_i > end_i:
        return 0
...
def find_trend_touch(candles: list[dict], line: dict) -> dict | None:
    """Latest wick touch on the line after p2, before break if any (0% near tolerance)."""
